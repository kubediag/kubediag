...
func TestRemoveFinalizer(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			desc:       "finalizer removed",
		},
		{
			finalizers: []string{"finalizer1", "finalizer2", "finalizer2"},
			toRemove:   "finalizer2",
			expected:   []string{"finalizer1"},
			desc:       "duplicated finalizers",
		},
	}

	for _, test := range tests {
		resultFinalizers := RemoveFinalizer(test.finalizers, test.toRemove)
		assert.Equal(t, test.expected, resultFinalizers, test.desc)
	}
}
